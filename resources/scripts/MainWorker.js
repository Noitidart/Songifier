		...
		if (aOkObj.ok) {
			...
			if (response.status.code == 1001) {
				...
				if (response.metadata.music.length === 1) {
					...
					try {
						aState.metdata.youtubeUrl = 'https://www.youtube.com/watch?v=' + response.metadata.music[0].external_metadata.youtube.vid;
					} catch(ignore) {}
					try {
						aState.metadata.spotifyUrl = 'https://geo.itunes.apple.com/gb/album/dudu/id' + response.metadata.music[0].external_metadata.spotify.track.id + '?mt=1&app=music';
					} catch(ignore) {}
					try {
						aState.metadata.itunesUrl = 'https://geo.itunes.apple.com/gb/album/dudu/id' + response.metadata.music[0].external_metadata.itunes.track.id + '?mt=1&app=music';
					} catch(ignore) {}
					gBsComm.postMessage('updateAttnBar', {
						aId,
						aState
